...
<script>
	...
	function init(){
		...
		var ingredientListStr = '${ingredientList}';
		ingredientListStr = ingredientListStr.substring( 1, ingredientListStr.length-1 );
		var ingredientList = ingredientListStr.split( ", " );
		var ingLabelList = $(".ingredient-checkbox label");
		
		ingredientList.forEach((a) => {
			for (var j = 0; j < ingLabelList.length; j++) {
				var $label = $(ingLabelList[j]);
				if ($label.text().includes(a)) {
...
<form name="contentForm">
	...
	<hr>
	<div style="width:100%;">
		<input type="text" name="title" class="title" placeholder="제목" value="${requestScope.postMap.TITLE}">
	</div>
    <div class="toolbar">
	...
	<!-- 편집기 -->
	<!-- ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ -->
    <div class="editor" contenteditable="true" spellcheck="false">${requestScope.postMap.CONTENT}</div>
